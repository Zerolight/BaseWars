...
function SWEP:PrimaryAttack()
	...
	local posang = self:GetOwner():GetAimVector()
	local tracedata = {}
	tracedata.start = pos
	...
	if tr.HitNonWorld then
		local ent = tr.Entity
		if ent:IsDoor() then
			if ent:GetNWEntity("DOwner") == self:GetOwner() then
				if not ent:GetNWBool("Locked") then
					if ent.NextLock and ent.NextLock >= CurTime() then
						self:GetOwner():PrintMessage(HUD_PRINTTALK, "This Door Cannot Be Locked For Another "..math.floor(ent.NextLock-CurTime()).." Seconds.")
						return
					end
					ent:Fire("Lock","",0)
