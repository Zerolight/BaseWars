...
if ( CLIENT ) then
	...
	SWEP.WepSelectIcon = surface.GetTextureID("backpack/weapons/c_models/c_pocket_watch/parts/c_pocket_watch.vtf")

	SWEP.Slot				= 1
	SWEP.SlotPos			= 1
	SWEP.IconLetter			= "G"

	surface.CreateFont( "DRfont", { font = "coolvertica", size = 13, weight = 500, antialias = true, shadow = false } )
...
ent:SetColor(self:GetOwner():GetColor())
ent:SetMaterial(self:GetOwner():GetMaterial())
ent:Spawn()
end

self:SendWeaponAnim( ACT_VM_DRAW )
if not self:GetOwner():GetNWBool("Status") == 3 or not self:GetOwner():GetNWBool("Status") == 4 or not self:GetOwner():GetNWBool("Status") == 1 then
self:GetOwner():SetNWBool(	"Status",			2)
end
return true
end
...
function SWEP:Holster()
	local worldmodel = ents.FindInSphere(self:GetOwner():GetPos(),0.6)
	for k, v in pairs(worldmodel) do
if v:GetClass() == "deadringer" and v:GetOwner() == self:GetOwner() then
v:Remove()
end
end
return true
...
if self:GetOwner():GetNWBool("CanAttack") == true and self:GetOwner():GetNWBool("Dead") == false and self:GetOwner():GetNWBool("Status") ~= 4 then

self:GetOwner():SetNWBool(	"Status",			1)
...
if self:GetOwner():GetNWBool("CanAttack") == true and self:GetOwner():GetNWBool("Dead") == false and self:GetOwner():GetNWBool("Status") ~= 4 then
self:GetOwner():SetNWBool(	"Status",			2)
self:EmitSound("buttons/blip1.wav", 100, 73)
else
return
end
end
