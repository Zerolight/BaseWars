...
function SWEP:Deploy()
	...
	self:EmitSound("katana/draw.wav")
return true
end
...
function SWEP:SecondaryAttack()
	...
	timer.Simple( 0.25, function() if IsValid( self ) then self:Bash() end end )
...
function SWEP:PrimaryAttack()
	self:SetNextSecondaryFire( CurTime() + self.Secondary.Delay )
	self:SetNextPrimaryFire( CurTime() + self.Primary.Delay )
	timer.Simple( 0.25, function() if IsValid( self ) then self:Slash() end end )
	self:SendWeaponAnim(ACT_VM_MISSCENTER)
	self:GetOwner():SetAnimation( PLAYER_ATTACK1 )
end

function SWEP:Slash()
 	local phtr = self:GetOwner():GetEyeTrace()
	local trace = util.GetPlayerTrace(self:GetOwner())
 	local tr = util.TraceLine(trace)
	local ph = phtr.Entity:GetPhysicsObject()
	if (self:GetOwner():GetPos() - tr.HitPos):Length() < 100 then
		if tr.Entity:IsPlayer() or string.find(tr.Entity:GetClass(),"npc") or string.find(tr.Entity:GetClass(),"prop_ragdoll") or tr.MatType == MAT_FLESH or tr.MatType == MAT_ALIENFLESH or tr.MatType == MAT_ANTLION then
			...
			util.Decal("ManhackCut", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)
				...
				self:EmitSound(SMetalHit[math.random(1,#SMetalHit)])
				elseif (tr.MatType == MAT_WOOD or tr.MatType == "MAT_FOLIAGE") then
				self:EmitSound(SWoodHit[math.random(1,#SWoodHit)])
				elseif (tr.MatType == MAT_GLASS) then
				self:EmitSound(SGlassHit[math.random(1,#SGlassHit)])
				...
				self:EmitSound(SGroundHit[math.random(1,#SGroundHit)])
				else
				self:EmitSound(SGroundHit[math.random(1,#SGroundHit)])
				end
			if not tr.HitWorld and not string.find(tr.Entity:GetClass(),"prop_static") then
				if SERVER then ph:ApplyForceCenter(self:GetOwner():GetAimVector()*5000)
...
function SWEP:Bash()
	...
	if (self:GetOwner():GetPos() - tr.HitPos):Length() < 100 then
		if tr.Entity:IsPlayer() or string.find(tr.Entity:GetClass(),"npc") or string.find(tr.Entity:GetClass(),"prop_ragdoll") or tr.MatType == MAT_FLESH then
			self:EmitSound( BFleshHit[math.random(1,#BFleshHit)] )
			tr.Entity:SetVelocity((phtr.Entity:GetPos() - self:GetOwner():GetPos()) * 20)
			if SERVER then
			tr.Entity:TakeDamage(10, self:GetOwner(), self)
			end
		else
			if (tr.MatType == MAT_METAL or tr.MatType == MAT_VENT or tr.MatType == MAT_COMPUTER) then
			self:EmitSound(BMetalHit[math.random(1,#BMetalHit)])
			elseif (tr.MatType == MAT_WOOD or tr.MatType == "MAT_FOLIAGE") then
			self:EmitSound(BWoodHit[math.random(1,#BWoodHit)])
			elseif (tr.MatType == MAT_GLASS) then
			self:EmitSound(BGlassHit[math.random(1,#BGlassHit)])
			...
			self:EmitSound(BGroundHit[math.random(1,#BGroundHit)])
			elseif (tr.MatType == MAT_CONCRETE) then
			self:EmitSound(BRockHit[math.random(1,#BRockHit)])
			else
			self:EmitSound(BGroundHit[math.random(1,#BGroundHit)])
			end
			if not tr.HitWorld and not string.find(tr.Entity:GetClass(),"prop_static") then
				if SERVER then ph:ApplyForceCenter(self:GetOwner():GetAimVector()*10000)
				tr.Entity:TakeDamage(10, self:GetOwner(), self)
				end
			end
		end
