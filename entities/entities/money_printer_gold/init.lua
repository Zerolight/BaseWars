...
function ENT:Initialize()
	...
	timer.Create( tostring(self), 90, 0, function() if IsValid( self ) then self:giveMoney() end end )
	timer.Create( tostring(self) .. "afkshutoff", 1200, 1, function() if IsValid( self ) then self:shutOff() end end )
	timer.Create( tostring(self) .. "notifyoff", 1000, 1, function() if IsValid( self ) then self:notifypl() end end )
	self:SetNWBool("sparking",false)
	self:SetNWInt("damage",400)
	...
	self.Payout = {100000, "Gold Printer"}
end
...
function ENT:giveMoney()
	...
	if(IsValid(ply) and not self.Inactive and self:IsPowered()) then
		...
		local amount = math.random( 5000, 5750 )
		if (self:GetNWInt("upgrade")==2) then
			amount = math.random( 10000, 12500 )
		elseif (self:GetNWInt("upgrade")==1) then
			amount = math.random( 7500, 7750 )
		end
...
function ENT:Reload()
	...
	self.Inactive = false
	self.NearInact = false
	self:SetColor(Color(255, 215, 0, 254))
