...
function ENT:Initialize()
	...
	timer.Create( tostring(self), 25, 0, function() if IsValid( self ) then self:giveMoney() end end )
	timer.Create( tostring(self) .. "afkshutoff", 1000, 1, function() if IsValid( self ) then self:shutOff() end end )
	timer.Create( tostring(self) .. "notifyoff", 750, 1, function() if IsValid( self ) then self:notifypl() end end )
...
function ENT:Reload()
	...
	timer.Create( tostring(self) .. "afkshutoff", 600, 1, function() if IsValid( self ) then self:shutOff() end end )
	timer.Destroy( tostring(self) .. "notifyoff")
	timer.Create( tostring(self) .. "notifyoff", 480, 1, function() if IsValid( self ) then self:notifypl() end end )
	self.Inactive = false
	self.NearInact = false
