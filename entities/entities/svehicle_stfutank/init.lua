...
function ENT:Think()
	if (IsValid(self.Pod) and IsValid(self.Pod:GetDriver()) and self.Pod:GetDriver():IsPlayer()) then

		local phys = self:GetPhysicsObject()
		phys:Wake()
		local ply = self.Pod:GetDriver()
		if (self:GetNWEntity("Driver")~=ply) then
		...
		if (IsValid(self.GunCore)) then
			local tankangles = self:GetAngles()
			...
			if ply:KeyDown(IN_ATTACK) and self.LastFired+5<CurTime() then
				self:FireShell()
		...
		if (self:GetNWEntity("Driver")~=self) then
			self:SetNWEntity("Driver", self)
		end
		self.GoForward = 0
		self:SetForceF(0)
		self.GoTurn = 0
	...
	if (IsValid(self.GunCore)) then
		if self.LastFired+1<CurTime() and self.LoadStage==0 then
			self.LoadStage=1
			self.GunCore:EmitSound(Sound("Town.d1_town_04_metal_solid_strain4"))
		elseif self.LastFired+2.5<CurTime() and self.LoadStage==1  then
			self.LoadStage=2
			self.GunCore:EmitSound(Sound("coast.crane_metal_groan"))
		elseif self.LastFired+4<CurTime() and self.LoadStage==2  then
			self.LoadStage=3
...
function ENT:OnRemove()
	timer.Destroy(tostring(self).."RemoveSelf")
	timer.Destroy(tostring(self).."Explodeeffect")
	timer.Destroy(tostring(self).."Pusheffect")
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if (self.GoForward==0 and self.GoTurn==0) then return SIM_NOTHING end
...
function ENT:FireShell()
	...
	if IsValid(object) then
		...
		object:SetPos(gunoffset+self:GetPos())
		object:SetAngles(firingangle)
		object:Spawn()
		object:GetPhysicsObject():SetVelocity(object:GetAngles():Up()*10000)
	end
	local effectdata = EffectData()
		effectdata:SetStart(self:GetPos()+gunoffset)
		effectdata:SetOrigin(self:GetPos()+gunoffset)
...
function ENT:PlateDied(ent, attacker)
	...
	elseif (self.PlateCount==0) then
		self:BlowRightTheFuckUp(attacker)
	end
end
...
function ENT:BlowRightTheFuckUp(destroyer)
	...
	if IsValid(self.Pod) then
		...
		if IsValid(driver) then
			...
			driver:TakeDamage(1000000, destroyer, self)
		end
	end
	self:Explode()
	timer.Create( tostring(self).."Explodeeffect", 0.15, math.random(3,5), function() if IsValid( self ) then self:Explode() end end )
	for i=1,8,1 do
		if (IsValid(self.Wheels[i])) then
			constraint.RemoveConstraints(self.Wheels[i], "AdvBallsocket")
		end
	end
	timer.Create( tostring(self).."Pusheffect", 0.1, 1, function() if IsValid( self ) then self:PushWheels() end end )
	if (IsValid(self.Pod)) then
	...
	timer.Create( tostring(self).."RemoveSelf", 120, 1, function() if IsValid( self ) then self:Remove() end end )
end
...
function ENT:Explode()
	local blastpos = self:GetPos()+Vector(math.random(-80,80),math.random(-80,80),math.random(-20,70))
	local effectdata = EffectData()
...
function ENT:PhysicsUpdate(phys)
	...
	if (IsValid(self.Plates[1])) then
		...
		self.Plates[1]:SetPos(plate1offset+self:GetPos())
	...
	if (IsValid(self.Plates[2])) then
		...
		self.Plates[2]:SetPos(plate2offset+self:GetPos())
	...
	if (IsValid(self.Plates[3])) then
		...
		self.Plates[3]:SetPos(plate3offset+self:GetPos())
	...
	if (IsValid(self.Plates[4])) then
		...
		self.Plates[4]:SetPos(plate4offset+self:GetPos())
	...
	if (IsValid(self.Plates[5])) then
		...
		self.Plates[5]:SetPos(plate5offset+self:GetPos())
	...
	if (IsValid(self.Plates[6])) then
		...
		self.Plates[6]:SetPos(plate6offset+self:GetPos())
	...
	if (IsValid(self.Plates[7])) then
		...
		self.Plates[7]:SetPos(plate7offset+self:GetPos())
	...
	if (IsValid(self.Plates[8])) then
		...
		self.Plates[8]:SetPos(plate8offset+self:GetPos())
	...
	if (IsValid(self.Body)) then
		...
		self.Body:SetPos(bodyoffset+self:GetPos())
	...
	if (IsValid(self.Engine)) then
		...
		self.Engine:SetPos(engineoffset+self:GetPos())
