...
function ENT:Initialize()
	...
	if self.Size==2 then
		self:SetModel("models/props/cs_militia/militiarock02.mdl")
...
function ENT:SetTarget(ent)
	...
	for i=1,20,1 do
		if not util.IsInWorld(spawnpos) then
			...
			if tracer.HitSky or not tracer.Hit then
				break
			end
		end
		if i==20 and not util.IsInWorld(spawnpos) then
...
function ENT:Destruct()
	...
	util.BlastDamage( self, self, self:GetPos(), 384, 150)
	...
	for i=1,5,1 do
		...
		local xpos = (math.sin(ang)*dist)+self:GetPos().x
		local ypos = (math.cos(ang)*dist)+self:GetPos().y
		local zpos = self:GetPos().z+200
		local trace = {}
			...
			trace.mask = COLLISION_GROUP_PLAYER
		tracer = util.TraceLine(trace)
		if util.IsInWorld(trace.start) and tracer.Hit and not tracer.HitSky then
...
function ENT:BigBang()
	...
	util.BlastDamage( self, self, self:GetPos(), 512, 250)
	...
	for i=1,5,1 do
		...
		local xpos = (math.sin(ang)*dist)+self:GetPos().x
		local ypos = (math.cos(ang)*dist)+self:GetPos().y
		local zpos = self:GetPos().z+math.random(0,128)
...
function ENT:PhysicsUpdate()
	self:GetPhysicsObject():SetVelocity(self.TargetPos * 6000)
	if self.DidHit then
