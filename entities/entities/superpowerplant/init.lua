...
function ENT:Initialize()
    ...
    self.Payout={CfgVars["supergeneratorcost"],"Super Generator"}
	local ply = (self.Owner or self:GetOwner())
	ply:GetTable().maxsupergenerator=ply:GetTable().maxsupergenerator + 1
	self.Inactive = false
	self.Powdist=1024
	...
	self:SetNWEntity("socket6",nil)
	self:SetNWEntity("socket7",nil)
	self:SetNWEntity("socket8",nil)
	self:SetNWEntity("socket9",nil)
	self:SetNWEntity("socket10",nil)
	self.scrap = false
