...
function ENT:Initialize()
	...
	self.Usable=false
	timer.Create( tostring(self) .. "lol", 2, 1, function() if IsValid( self ) then self:UsableGet() end end )
	self.Time = CurTime()
end
...
function ENT:Use(activator,caller)
	if (caller:Health()<caller:GetMaxHealth() and self.Usable==true) then
		caller:SetHealth(caller:GetMaxHealth())
		if caller:GetTable().PoisonDuration>0 and math.random()>0.5 then
