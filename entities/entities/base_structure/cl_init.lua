...
function ENT:Think()
	if tonumber(LocalPlayer():GetInfo("bw_showsparks"))==1 then
		...
		if self:GetNWBool("sparking") and self.SparkPos~=nil and self.dospark==0 then
			self.dospark=self.dospark+1
			local ang = self:GetAngles()
			local spos = self.SparkPos
			local effectdata = EffectData()
				effectdata:SetOrigin( self:GetPos()+ang:Forward()*spos.x+ang:Right()*spos.y+ang:Up()*spos.z)
...
function ENT:Draw( flags )
	...
	if self.SparkPos~=nil and tonumber(LocalPlayer():GetInfo("bw_showsparks"))==0 then
		local ang = self:GetAngles()
		local spos = self.SparkPos
		local dotpos = self:GetPos()+ang:Forward()*spos.x+ang:Right()*spos.y+ang:Up()*spos.z
...
function ENT:DrawEntityOutline( size )
	...
	if self.HealthOffset~=nil then offsetright=self:GetAngles():Right()*self.HealthOffset end
	local pos = self:GetPos()+self:GetAngles():Up()*self.HealthRing[3]+offsetright
	...
	cam.Start3D2D(pos, eang, 1)
		local health = 16*math.Round(22.5*(self:GetNWInt("damage")/self.HealthRing[1]))
		if health<=90 then
