...
function ENT:Initialize()
	...
	timer.Create( tostring(self), 100, 0, function() if IsValid( self ) then self:giveMoney() end end )
...
function ENT:giveMoney()
	...
	if(IsValid(ply) and not self.Inactive and self:IsPowered()) then
		...
		local amount = math.random( 20000, 22500 )
		if (self:GetNWInt("upgrade")==2) then
			amount = math.random( 50000, 52500 )
		elseif (self:GetNWInt("upgrade")==1) then
			amount = math.random( 35000, 37500 )
		end
