...
surface.CreateFont( "Upgradedeath", { font = "csd", size = ScreenScale( 20 ), weight = 500, antialias = true, shadow = true } )
surface.CreateFont( "Beamsymbol", { font = "HalfLife2", size = ScreenScale(32), weight = 500, antialias = true, shadow = true } );
...
local function RecvPlrKilledByPlr()
...
net.Receive( "PlrKilledPlr", RecvPlrKilledByPlr )

local function RecvPlrKilledSelf()
	local victim 	= net.ReadEntity();
	local inflictor = net.ReadEntity();
	local class	= net.ReadString();
	local headshot = net.ReadBool()
...
net.Receive( "PlrKilledSelf", RecvPlrKilledSelf )

local function RecvPlrKilled()
	local victim 	= net.ReadEntity();
	local inflictor	= net.ReadEntity();
...
net.Receive( "PlrKilled", RecvPlrKilled )
