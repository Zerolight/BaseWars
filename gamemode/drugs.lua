...
function BurnPlayer(pl, duration, attacker, inflictor)
	if IsValid(pl) then
		...
		if pl:GetTable().BurnDuration>=4 and not pl:IsOnFire() then
...
function CheckBurn(pl)
	...
	if pl:GetTable().BurnDuration<=2 and pl:IsOnFire() then
		pl:Extinguish()
	end
...
function Randup(ply, time)
	if ply:GetTable().Regened and ply:GetTable().Roided and ply:GetTable().MagicBulleted and ply:GetTable().Amp and ply:GetTable().Painkillered and ply:GetTable().Antidoted and ply:GetTable().Mirror and ply:GetTable().Focus and ply:GetTable().Adrenalined and ply:GetTable().DoubleJumped and ply:GetTable().ShockWaved and ply:GetTable().DoubleTapped and ply:GetTable().Leeched and ply:GetTable().KnockBacked and ply:GetTable().ArmorPiercered then
		return
	end
	local gaveadrug = false
	local i=0
	local duration = time or CfgVars["randomduration"]
	while not gaveadrug and i<50 do
		i=i+1
		local rand = math.random(1,15)
		gaveadrug = true
		if (ply:GetTable().Regened ~= true and rand==1) then
			Regenup(ply, duration)
		elseif (ply:GetTable().Roided ~= true and rand==2) then
			Roidup(ply, duration)
		elseif (ply:GetTable().MagicBulleted ~= true and rand==3) then
			MagicBulletup(ply, duration)
		elseif (ply:GetTable().Amp ~= true and rand==4) then
			Ampup(ply, duration)
		elseif (ply:GetTable().PainKillered ~= true and rand==5) then
			PainKillerup(ply, duration)
		elseif (ply:GetTable().Antidoted ~= true and rand==6) then
			Antidoteup(ply, duration)
		elseif (ply:GetTable().Mirror ~= true and rand==7) then
			Mirrorup(ply, duration)
		elseif (ply:GetTable().Focus ~= true and rand==8) then
			Focusup(ply, duration)
		elseif (ply:GetTable().Adrenalined ~= true and rand==9) then
			Adrenalineup(ply, duration)
		elseif (ply:GetTable().DoubleJumped ~= true and rand==10) then
			DoubleJumpup(ply, duration)
		elseif (ply:GetTable().ShockWaved ~= true and rand==11) then
			ShockWaveup(ply, duration)
		elseif (ply:GetTable().DoubleTapped ~= true and rand==12) then
			DoubleTapup(ply, duration)
		elseif (ply:GetTable().Leeched ~= true and rand==13) then
			Leechup(ply, duration)
		elseif (ply:GetTable().Knockbacked ~= true and rand==14) then
			Knockbackup(ply, duration)
		elseif (ply:GetTable().ArmorPiercered ~= true and rand==15) then
...
function Regenup(pl, duration)
	...
	Notify( pl, 3, 3, "You have taken Regeneration." );
	local IDSteam = string.gsub(pl:SteamID(), ":", "")
...
function Regentick(pl)
	local IDSteam = string.gsub(pl:SteamID(), ":", "")
	if (not pl:GetTable().Regened or pl:Health()<=0) then
...
function Mirrorup(pl, duration)
	...
	Notify( pl, 3, 3, "You have taken Reflect." );
	local IDSteam = string.gsub(pl:SteamID(), ":", "")
...
function UnAdrenalineup(pl)
	...
	if pl:Health()~=0 and pl:GetMaxHealth()~=0 then
		pl:SetMaxHealth(pl:GetMaxHealth()/1.5)
		pl:SetHealth(pl:Health()/1.5)
	end
end
...
function DoubleJumpup(pl, duration)
	...
	Notify( pl, 3, 3, "You have taken Double Jump." );
	local IDSteam = string.gsub(pl:SteamID(), ":", "")
