...
function DoorsSetup()
	local mapProps = Properties[game.GetMap()]
	for k,v in pairs (ents.GetAll()) do
		if table.HasValue(DoorType, v:GetClass()) then
			v:SetNWInt("State", 0)
			if mapProps then
				for a,b in pairs(mapProps) do
...
function pmeta:DoorControl()
	...
	if not table.HasValue(DoorType, tar:GetClass()) then return end
		if tar:GetNWInt("State") == 0 and not self.ODR then
	...
	elseif tar:GetNWInt("State") == 1 and tar:GetNWEntity("DOwner") == self then
		...
		self:PrintMessage( HUD_PRINTTALK, "You just sold "..tar:GetNWInt("Price", PPrice)/PPrice.." Doors for $"..math.floor(tar:GetNWInt("Price", 75)/1.75))
		self.ODR = false
	for k,v in pairs(ents.GetAll()) do
		if v:GetNWString("Property") == tar:GetNWString("Property") then
			v:SetNWInt("State", 0)
...
hook.Add("PlayerDisconnect", "DoorClearup", function(ply)
	for k,v in pairs(ents.GetAll()) do
		if not table.HasValue(DoorType, v:GetClass()) then continue end
